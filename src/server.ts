...
// CDP-based raw-eval selector generator to avoid bundler helper injection errors
async function generateSelectorAtPointWithDebugCDP(x: number, y: number): Promise<{ selector: string | null, candidates: string[], promotedTag: string | null, actionableTag: string | null, evalError?: string | null }> {
  try {
    const client = await getCDPClient();
    const expr = `(() => {\n      try {\n        const startTime = Date.now();\n        const TIMEOUT_MS = 200;\n        const isTimedOut = () => Date.now() - startTime > TIMEOUT_MS;\n        const uniq = (arr) => Array.from(new Set((arr || []).filter(Boolean)));\n        const cssEscape = (str) => {\n          if (!str) return str;\n          let escaped = (window.CSS && window.CSS.escape) ? window.CSS.escape(str) : String(str).replace(/[^a-zA-Z0-9_-]/g, (r) => \\\"\\\\\\\" + r);\n          escaped = escaped.replace(/\\\\([0-9a-f]+)\\s+(-)/gi, \\\"\\\\$1$2\\\");\n          return escaped;\n        };\n        const isAcceptable = (sel, el) => {\n          if (!sel) return false;\n          try { const list = document.querySelectorAll(sel); if (list.length === 1 && list[0] === el) return true; } catch {}\n          try { return el.matches(sel); } catch { return false; }\n        };\n        const getActionableAncestor = (el) => {\n          let cur = el; let depth = 0;\n          while (cur && depth < 6) {\n            const tag = cur.tagName.toLowerCase();\n            const role = cur.getAttribute('role');\n            const tabindex = cur.getAttribute && cur.getAttribute('tabindex');\n            const onclick = cur.onclick;\n            let pointer = false;\n            try { const style = window.getComputedStyle(cur); pointer = style ? style.cursor === 'pointer' : false; } catch {}\n            if (tag === 'button' || tag === 'a' || tag === 'label' || tag === 'input' || tag === 'textarea' || tag === 'select' || role === 'button' || (tabindex !== null && Number(tabindex) >= 0) || onclick || pointer) return cur;\n            cur = cur.parentElement; depth++;\n          }\n          return null;\n        };\n        const promoteFromSVG = (el) => {\n          let cur = el;\n          while (cur && ['path','svg','g','use'].includes(cur.tagName.toLowerCase())) { if (!cur.parentElement) break; cur = cur.parentElement; }\n          return { element: cur, promotedTag: cur ? cur.tagName.toLowerCase() : null };\n        };\n        const buildCandidates = (el) => {\n          const cands = []; const tag = el.tagName.toLowerCase();\n          const tests = ['data-testid','data-test','data-cy','data-automation-id','data-qa','data-test-id'];\n          for (const a of tests) { try { const v = el.getAttribute(a); if (v) { cands.push('[' + a + '=\\\"' + cssEscape(v) + '\\\"]', tag + '[' + a + '=\\\"' + cssEscape(v) + '\\\"]'); } } catch {} }\n          try { const id = el.id; if (id) cands.push('#' + cssEscape(id)); } catch {}\n          let role = null, aria = null;\n          try { role = el.getAttribute('role'); aria = el.getAttribute('aria-label'); } catch {}\n          if (aria) cands.push('[role=\\\"' + (role || tag) + '\\\"][aria-label=\\\"' + cssEscape(aria) + '\\\"]', tag + '[aria-label=\\\"' + cssEscape(aria) + '\\\"]');\n          try { const nameAttr = el.getAttribute('name'); if (nameAttr) cands.push(tag + '[name=\\\"' + cssEscape(nameAttr) + '\\\"]'); } catch {}\n          try { const placeholder = el.getAttribute('placeholder'); if (placeholder) cands.push(tag + '[placeholder=\\\"' + cssEscape(placeholder) + '\\\"]'); } catch {}\n          try { const typeAttr = el.getAttribute('type'); if (typeAttr) cands.push(tag + '[type=\\\"' + cssEscape(typeAttr) + '\\\"]'); } catch {}\n          try { const forAttr = el.getAttribute('for'); if (forAttr) cands.push(tag + '[for=\\\"' + cssEscape(forAttr) + '\\\"]'); } catch {}\n          const ariaAttrs = ['aria-labelledby','aria-describedby','aria-controls','aria-expanded','aria-selected'];\n          for (const a of ariaAttrs) { try { const v = el.getAttribute(a); if (v) cands.push(tag + '[' + a + '=\\\"' + cssEscape(v) + '\\\"]'); } catch {} }\n          try { const cls = el.className; if (cls && typeof cls === 'string') { const classes = cls.split(/\\s+/).filter(Boolean); if (classes.length) { const max = Math.min(classes.length, 2); for (let i=0;i<max;i++){ const seg = classes.slice(0,i+1).map(cssEscape).join('.'); cands.push(tag + '.' + seg); } } } } catch {}\n          try { if (el.parentElement) { const siblings = Array.from(el.parentElement.children).filter(n => n.tagName === el.tagName); const idx = siblings.indexOf(el) + 1; cands.push(tag + ':nth-of-type(' + idx + ')'); } } catch {}\n          return uniq(cands);\n        };\n        let baseEl = document.elementFromPoint(${x}, ${y});\n        if (!baseEl) return { selector: null, candidates: [], promotedTag: null, actionableTag: null };\n        const promoted = promoteFromSVG(baseEl);\n        const nonSvgEl = promoted.element;\n        const actionable = getActionableAncestor(nonSvgEl) || nonSvgEl;\n        const directCands = buildCandidates(actionable);\n        for (const sel of directCands) { if (isAcceptable(sel, actionable)) { return { selector: sel, candidates: directCands, promotedTag: promoted.promotedTag, actionableTag: actionable.tagName.toLowerCase() }; } }\n        let current = actionable.parentElement; let depth = 0;\n        while (current && current !== document.body && depth < 6 && !isTimedOut()) {\n          const ancCands = buildCandidates(current); const leafCands = buildCandidates(actionable);\n          for (const a of ancCands) { for (const l of leafCands) { const combined = a + ' ' + l; if (isAcceptable(combined, actionable)) { return { selector: combined, candidates: ancCands.concat(leafCands), promotedTag: promoted.promotedTag, actionableTag: actionable.tagName.toLowerCase() }; } } }\n          current = current.parentElement; depth++;\n        }\n        let node = actionable; const path = [];\n        while (node && node !== document.body && path.length < 8 && !isTimedOut()) {\n          if (node.parentElement) { const sibs = Array.from(node.parentElement.children).filter(n => n.tagName === node.tagName); const idx = sibs.indexOf(node) + 1; path.unshift(node.tagName.toLowerCase() + ':nth-of-type(' + idx + ')'); const test = path.join(' > '); if (isAcceptable(test, actionable)) { return { selector: test, candidates: [], promotedTag: promoted.promotedTag, actionableTag: actionable.tagName.toLowerCase() }; } }\n          node = node.parentElement;\n        }\n        return { selector: null, candidates: directCands, promotedTag: promoted.promotedTag, actionableTag: actionable.tagName.toLowerCase() };\n      } catch (e) {\n        return { selector: null, candidates: [], promotedTag: null, actionableTag: null, evalError: String(e && e.message || e) };\n      }\n    })()`;
    const { result, exceptionDetails } = await client.send('Runtime.evaluate', { expression: expr, returnByValue: true });
    if (exceptionDetails) {
      return { selector: null, candidates: [], promotedTag: null, actionableTag: null, evalError: exceptionDetails.text || 'eval-exception' };
    }
    return (result && result.value) || { selector: null, candidates: [], promotedTag: null, actionableTag: null };
  } catch (e) {
    return { selector: null, candidates: [], promotedTag: null, actionableTag: null, evalError: (e as Error).message };
  }
}
